...
static uint8_t reorient_count; // Prevent getting stuck in reorient loop. Increased every frame in REORIENT, decreased when in TRAVEL
...
float calc_angle_diff(float angle_1, float angle_2) {
    float diff = fmodf(angle_2 - angle_1, 2 * M_PI);
    if (diff > M_PI) {
        diff -= 2 * M_PI;
    } else if (diff < -M_PI) {
        diff += 2 * M_PI;
    ...
    switch (current_state) {
        case TRAVEL:
            if (!boundary_ok) {
                ...
                // Try to counter "drifting" out of corners when turning while moving forward
        ...
        case AVOID_BOUNDARY:
            // Turn in place, away from boundary
